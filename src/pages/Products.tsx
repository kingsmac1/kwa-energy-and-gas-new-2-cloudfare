        ...
        <Stagger className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {PRODUCTS.map((p) => (
            <StaggerItem key={p.id}>
              <Link to={`/products/${p.id}`} className="group flex h-full flex-col overflow-hidden rounded-3xl border border-[var(--brand-dark)]/10 bg-[var(--brand-bg)] transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-[var(--brand-dark)]/5">
