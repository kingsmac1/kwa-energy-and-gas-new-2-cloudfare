  ...
  return (
    <>
      ...
      <Section tone="white" className="!pt-0">
        <Link to={`/blog/${featured.slug}`} className="group grid gap-8 overflow-hidden rounded-3xl bg-[var(--brand-bg)] lg:grid-cols-2">
        ...
        <Stagger className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {rest.map((post) => (
            <StaggerItem key={post.slug}>
              <Link to={`/blog/${post.slug}`} className="group block overflow-hidden rounded-3xl bg-[var(--brand-bg)] transition hover:shadow-xl hover:shadow-[var(--brand-dark)]/5">
