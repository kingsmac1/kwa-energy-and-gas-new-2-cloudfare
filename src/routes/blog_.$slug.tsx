...
export const Route = createFileRoute("/blog_/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!post) return { meta: [{ title: "Article not found — Kwa Gas and Energy" }] };
    return {
      meta: [
        { title: `${post.title} — Kwa Gas and Energy` },
    ...
    <>
      ...
      <Section tone="white">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {others.map((p) => (
            <Link key={p.slug} to="/blog/$slug" params={{ slug: p.slug }} className="group overflow-hidden rounded-3xl bg-[var(--brand-bg)]">
